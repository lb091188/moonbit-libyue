...
- [~] 发布 mooncakes 包(API 面稳定后) — **发布就绪,暂不发布(2026-09-12 确认)**:一切就绪(moon.mod.json name/version、README 双语 `moon add NoahLiu/moonbit-libyue` 用法、API 面已核对);`moon publish` 实测受阻于账号凭据(`~/.moon/credentials.json` 不存在),`moon login` 为所有者的交互式账号授权,无法代执行。**剩余动作(仅一步)**:所有者执行 `moon login && moon publish`,完成后把本项改为 [x]
