...
- 连发滚轮撕裂(真机「分裂」截图:猛拨滚轮后原生输入框组新旧位置叠影):输入消息优先级高于 WM_PAINT——快拨时多个 WM_MOUSEWHEEL 在任何重绘之前排队处理,每个 OnScroll 先 ScrollWindowEx blit 再失效暴露边带;**下一个 blit 的源区里含着上一个「已失效但尚未重画」的边带**,把陈旧像素搬进视口中段,逐拍复合成大面错位。此前探针未复现是因为程序化逐拍滚动(定时器每拍 40px)在拍间留了重绘窗口,而真实输入是连发。修复(fork 2bf490f7):ScrollPixels 的 ScrollWindowEx 之后紧跟 `UpdateWindow(hwnd)` 同步补画暴露边带——任何后续 blit 读到的都是新鲜表面;每档滚轮的边带重画工作量不变(本来就要画),只是从异步改同步。方法论:①PostMessage WM_MOUSEWHEEL 直投 hwnd 可确定性复现连发时序(SendInput 注入不可靠——目标窗口非前台时整体被丢,脚本点击激活又可能落点窗外激活了别的窗口);②**用户在场的真手联测效率远高于盲跑脚本**:真手悬停/滚轮 + shim 文件打点(root/origin/set_visible/set_style left/top 全链)+ 用户口述现象,一轮即锁定三个根因。
