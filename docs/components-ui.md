...
| `input_t(text?, password?, margin?, width?, height?, clearable?)` | bordered / password / clearable | outer self-drawn 1px border (focus turns primary), inner Entry stripped of native border & inner shadow via `set_borderless`; clearable=true shows ✕ on hover when non-empty, click to clear |
...
| `textarea_t(text?, width?, height?, margin?, on_input?, clearable?)` | multi-line input: self-drawn border (focus turns theme primary) + 8px inset, inner TextEdit with native border removed; overflow scrolls per platform; clearable=true shows ✕ on hover when non-empty, click to clear (on_input receives "") |
