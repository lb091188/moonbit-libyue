...
9. **每完成一个批次的需求就提交到仓库**——一个批次=一组内聚的改动(一个功能/一次修复/一批文档),完成即 `git commit` 并推送 origin,不积压到工作区;单批改动过大时按内聚单元拆成多批(一批一个主题,代码权责清晰),逐批提交,避免多主题挤进一条;提交信息沿用「【标签】范围:说明」中文格式(【新增】/【修复】/【文档】/【构建】),主题行控制在百字上下:改动内容一句 + 验证结果一句,原理/根因/实测过程以专门文档为准,不在提交信息里复述。
