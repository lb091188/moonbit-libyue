...
- tooltip 自绘弹层·Windows 现状与断点(移交 Windows 主机会话):自绘改 Popover 弹层后 Windows 真机两次反馈——每悬停新建实例闪退(已改全局单例复用,Popover 在 Windows 是独立小窗,库内活动弹层指针/8 秒自动关/点外鼠标钩子都绑单例,见 yue_mbt_popover_close 注释),单例化后不再崩但气泡仍不出现。断点已缩小到 Windows 特有环节:组件层 show 代码(事件注册→500ms 定时→Popover 构造→set_content→show_relative_to)在 Linux 宿主机插桩+截图验证全链路通(Linux 走 GTK Popover 原生实现),但这对 Windows 无证据价值——Windows 的 yue_mbt_popover_show_relative_to 是独立小窗+SetWindowPos+SW_SHOWNOACTIVATE 的替代实现,高嫌疑:弹层窗口创建/不激活显示路径(无 v6 清单环境下 ShowWindow 行为)/锚点 HWND 取值。Linux 宿主机无法编译 OS_WIN 分支,插桩探针需在 Windows 会话本地跑(EnumWindows 数弹层窗口+打 rect/visible)。教训入档:Linux 侧验证(布局探针/链路插桩)不能外推 Windows,GTK 全量重排与 Win32 消息链差异会把 Windows 独有病完全掩掉——合并调度 632f440 即因此回滚(Linux 逐位无回归,Windows 切页病疑似复发)。
