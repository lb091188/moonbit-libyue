...
> 状态：**B1-B8 全部实施完成**(单批提交见 git 历史);计划中「用户执行」的真机项(休眠/锁屏/拔插电源/断联网/双开/重启拉起的桌面侧确认与 Windows 真机)待验,已逐项标注于 TODO.md 对应验收列。
> 本计划由只读调研产出：全部代码引用按当时 HEAD 逐条读实。
