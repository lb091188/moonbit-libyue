...
- **autocomplete 弹层点击即被激活,点选候选项落空(2026-09-17 代码审计推断,待真机复验)**:替代窗口的 `no_activate` 与显示时的 `SWP_NOACTIVATE` 只挡「弹出不激活」,挡不住「点击激活」——鼠标按下候选行时弹窗被激活为前台窗口,Entry 收 `EN_KILLFOCUS`,组件按失焦收起弹层,点击落空且焦点被抢。修复:shim `yue_mbt_popover_new` Windows 分支给弹窗 HWND 补 `WS_EX_NOACTIVATE`,点击直达行内容、窗口永不成为前台,焦点全程留在输入框。
- **autocomplete 弹层定位到屏幕外不显示(2026-09-17 Win10 真机实测)**:输入弹层后无窗口出现,shim 日志 `popover: shown at (-595616576, 899)`——`View::GetBoundsInScreen` 在嵌套滚动容器下把垃圾偏移累进 x,与 Linux 同型 bug(Linux 已补 `patch_linux_view_bounds_in_screen`,Windows 上游实现同样不可信);y 值正常说明仅横向累加链坏。修复:shim `yue_mbt_popover_show_relative_to` Windows 分支锚点坐标改取原生子控件 HWND 的 `GetWindowRect`(Entry 是 SubwinView,`dynamic_cast` 取 HWND,系统真相不受布局累加污染;坐标系与 `SetWindowPos` 一致按物理像素),取不到 HWND 再回退 `GetBoundsInScreen`。
