...
Windows 上 `scroll()` 的默认形态（overlay=true 且未显式 policy）即走这条自绘悬浮细条；本组件是其**显式选用**入口——需要在 Linux/macOS 上使用同款自绘悬浮细条（替代平台悬浮样式）时采用，三平台形态统一。行为：隐藏平台滚动条，自绘主题色细条叠在内容右缘——滚动/悬停浮现，停顿约 1 秒两级渐隐，可沿轨道拖拽，窄条上滚轮转发给内容滚动。
