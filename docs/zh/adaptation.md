...
- Windows 原生子控件滚动摆位与两个 yoga 坑(表单页滚动真机反馈「原生输入框影响顶部深浅切换区渲染」):①原生子控件(Entry/TextEdit 的 HWND)由 libyue 原生的 `ScrollImpl::Layout` content 分配链同步摆位(`SubwinView::SizeAllocate` → SetWindowPos + SetWindowRgn 视口裁剪),滚动**不需要**外部兜底——declarative `scroll()` 早期挂的 `on_scroll → update_layout(holder)` 在 update_layout 改为根级遍历后反而引入分配中间态(根级重算触发页容器重新分配、视口塌陷),已移除。②`flex:1` 的 basis 默认 auto,按内容自然高度测量——内容挂载期(holder 尚无父)的 0×0 yoga 计算会把测量缓存污染成 0,滚动视口高度塌陷(实测 356→0,原生控件裁剪区恒空、HWND 永不摆位);scroll() 现显式补 `flexbasis: 0`(CSS flex:1 1 0 语义)。③窗口 content view 是 yoga 根,height 默认 auto 时容器没有「剩余空间」可分,flex:1 子高度塌陷;根的 percent 100% 无参照不生效,`Window::set_content` 改为设绝对值 + `on_size_changed` 跟随窗口 resize(同值不标脏无循环)。探针(原生 HWND 屏幕矩形 + SubwinView::SA 打点)验证滚动后 HWND setpos 跟随、裁剪区非空。
...
- 显隐页切换消失·三层嵌套残余场景(showcase 结构:页容器 set_visible 切页 + scroll + section + tabs_t,真机仍复现):页签切换时,`Container::Layout` 的 dirty 自愈分支(view.cc 里自带 TODO 注释的那条)会用 display 切换中间态的 yoga 值分配外层容器——并列 flex:1 的页容器被按「scroll 内容测量中间态」分配成压缩高度(实测 210→56),且此后无法自救:自愈传播链断在 Scroll(非 Container),根级重算的 SetBounds 链又断在「尺寸未变的中间容器」(hbox 等尺寸相同 → ViewImpl::SizeAllocate 早退 → 不向下触发子级 UpdateChildBounds)。后果:WM_PAINT 的 dirty 被压缩的页容器裁成 24 高碎片,与页区域(74,278,512,50)不相交,`DrawChild` 的 `child_dirty.IsEmpty()` 整块跳过——on_draw 不触发、bounds 却正常。已试无效(均实测):根级重算 ×N、反转显隐顺序(先 true 后 false)、set_visible 内联根重算(shim 侧)、叶子层显隐(显隐只切页内容)、page_c 的 flexbasis 置 0(CSS flex:1 1 0 语义)。结论:根因在 libyue win 的 yoga 集成本身(dirty 自愈用过期布局 + Scroll 断链的双重断裂),yue/shim 层外部修补打不穿,需 fork 侧根治。fork 侧补丁已落(fork ba479418):`Container::UpdateChildBounds` 对可见子容器**无条件递归下钻**——子容器尺寸未变时 SetBounds 早退、SizeAllocate→Layout 级联断链,其子树整轮错过分配;下钻只把「根级最新 yoga 结果」重新分发,不自带约束,本机交互验证切页/页签/图表各页渲染正常。备选方向之一「非 root 容器分配前强制 YGNodeCalculateLayout(以自身 bounds 为约束)**实测有害已否决**:布局早期轮次里容器自身 bounds 还可能是 0,把它当 owner 尺寸算子树会把 0 下推给整棵子树——重启后首次切页整页空白的回归即此(下钻与强制重算二分定位确认)。当前 20babb5 修复对「tabs_t 直接位于 scroll 内容」场景(无页容器层)完全有效;showcase 三层场景待真机复验。
- Linux tabs_t 切页塌陷(auto 高链既有病,非回归):Linux 上 tabs_t(或任何 flex:1 子挂 auto 高父容器的结构)显隐切页后,flex 子被分配塌成 marginBottom 值——探针实测 tabs outer 65→10(marginBottom=10.0),容器内部 head 仍 36(分配值塌而内容不塌)。四组对照全部同样塌:页容器 basis 0 / 平台分化(不写 basis)、去掉 tabs_t 的 refresh_layout、去掉 shim set_visible 的根级 Layout、检出 2026-09-23 显隐修复链之前的旧版(20babb5^);不经过 tabs_t、手工复刻的 outer(head 36 + 两个 flex 页)结构切页同样塌。结论:与 MoonBit 层(tabs_t / basis / refresh_layout)和 shim 根级重算均无关,根因在 libyue GTK yoga 对「auto 高父容器 + flex 子显隐」的分配链(GTK 全量重排不掩此病,与 257 条 Windows 独有病是两端各自爆发的孪生根因)。basis 0 的平台分化(Windows 保留、其余平台不写)保留,但它治的是 Windows 压缩病,与本症状无关。未修;方向:父链给显式高度(治标)或 fork 侧查 GTK auto 高测量污染(治本)。验证方式:探针 mount_into + get_bounds 时序测量(挂载 1.2s 测初始,set_visible 两页后再 1.2s 复测)。

- resize 卡顿/拖拽不动·定位(探针+源码,未动刀):为治视口塌陷给 yoga 根挂的 `on_size_changed → set_style("height")` 跟随(0353d08)被疑为拖拽卡顿根因,两项候选刀法实测均无收益——①「同值早退」:fork 源码坐实 `View::SetStyleProperty(name, float)` 直通 `SetYogaProperty → YGNodeStyleSetHeight`,库侧无同值比较(0353d08 注释「同值不标脏」的说法在库侧不成立);但同值 `set_style("height")` 20 次仅 3.58ms(0.18ms/次),且拖拽中 height 每帧真变、同值路径根本不热,补早退无收益。②「定时器合并」:探针(root flex:1 + scroll + 20 Entry)连续 20 次 `set_content_size` 后 1s 内 `on_size_changed` 只派发 1 次(debug/release 一致)——GTK 层已把连续 configure 合并,apply_height 链 1:1 无连锁反压,合并无收益。resize 单次同步链成本 ~2.5ms(debug,20 Entry 全窗重排),与 set_visible 早退/tabs_t 改动不在同一路径(那两项不经 resize),故「卡顿/拖不动」不是本仓这两天改动的回归;真机交互级冻结超出程序化探针能力,需真机复现(edge-grip 拖拽 vs 程序化 set_content_size 是两条路,后者每次必过 GTK resize 同步链)。Windows 卡顿同理待真机(全平台 fork ba479418 递归下钻为共用因素)。

- 原生 tooltip 在 Windows 环境全不弹(真机三处对照二分):exe 无 comctl32 v6 清单(同 TaskDialog 降级根因),State::PlatformInit 的 InitCommonControlsEx(ICC_WIN95_CLASSES) 有走、TooltipHost(TOOLTIPS_CLASS 进程级单例)与 TTM_ADDTOOL 链路源码读穿无断点——区外自绘容器 / scroll 内自绘容器 / 原生 Entry 三处 tooltip 全不弹,排除坐标系/scroll 裁剪/自绘路径。修复:放弃原生路径,`attach_tooltip`/`tooltip_t`/icons 的 tip 统一改自绘——悬停 500ms 延时在控件下方弹主题底色文字气泡(尺寸 AttributedText 测宽 + padding),移开/按下即收(气泡载体两代演进见下两条);事件挂 content 自身视图(包内 FFI,View 不实现 ViewLike),挂宿主容器会踩 hover_group 同款「子控件事件独占」洞。
- tooltip 自绘弹层·Windows 闪退与不显示·双层根因已修(Windows 本机插桩探针定位):「单例化后不再崩但气泡仍不出现」实为两层叠加。①**闭包无 GC root 即崩**:`attach_tooltip` 的 enter/leave/down 闭包直接经 ffi 传 C,未按 yue/events.mbt 注册表补 `keep_alive(fn() { ignore(closure) })`——MoonBit GC 无 root 时回收/移动闭包,C 侧 Connect 的 lambda 持悬空指针,事件触发即访问违例(本机探针连跑 5 轮崩 4 轮,exit 0xC0000005/0xC0000409 交替、崩点随 GC 时机漂移、加打点改变分配后复现性即变,典型 UB;分事件二分:只挂 enter 即崩、只挂 leave/down 干净)。**凡闭包经 ffi 传 C 必 keep_alive**,events.mbt 全部合规,attach_tooltip 是唯一漏网。②**自绘锚点错位**:shim 的 show_relative_to 对取不到 HWND 的自绘视图回退 `View::GetBoundsInScreen`,Windows 嵌套容器下偏移垃圾(本轮实测气泡 visible=1 却被摆到屏幕左上角 (68,72),用户视角就是「不显示」;此前实测还有 -5.9 亿巨幅偏移)。修复:自绘锚点改 `ViewImpl::size_allocation`(即相对顶层窗口客户区的绝对坐标,事件换算 GetPosInView 同源)+ 顶层 hwnd `ClientToScreen` 换屏幕物理像素;探针实测 scroll 内/区外两锚点气泡均弹在按钮正下方居中,单例复用(reuse)路径同验,抓屏视觉取证。方法论:Windows 会话本地真实鼠标(SetCursorPos)驱动全链 + shim stderr 打点 + 抓屏,一轮定位两层根因;**探针窗口必须 set_always_on_top**,否则被 IDE 大窗遮住,悬停全落空、enter 一次不触发,白跑;shim 改后 moon 增量不重链的老坑(见链接参数节)在探针轮换里同样要删 `_build` 产物 exe。方法论教训(前轮已证,继续有效):Linux 侧验证(布局探针/链路插桩)不能外推 Windows——GTK 全量重排与 Win32 消息链差异会把 Windows 独有病完全掩掉,合并调度 632f440 即因此回滚。后续:Popover 独立小窗整链(窗口创建/单例管理/8 秒自动关/点外鼠标钩子)真机渲染卡 500ms~1s,不满足「中低配无感、对齐 GTK3」标准,整链废弃改窗口内自绘覆盖层(见下条);keep_alive 必补与锚点坐标两条结论在新方案原样沿用。
- tooltip 第三代·窗口内自绘覆盖层(用户拍板改「message(toast)那种内部自绘小消息」形式,不再做子窗口):气泡=挂**锚点顶层根容器**的 absolute 定位自绘视图(themed_container,on_draw 主题底 + 1px 描边 + AttributedText 文字),创建时测好尺寸常驻隐藏,悬停 500ms set_timeout 到点后 set_style(left/top)+set_visible+schedule_paint,移开/按下即收——零新窗口零钩子零焦点变更,悬停后仅一次视图显隐+重绘(旧方案「渲染卡 500ms~1s」的本体是 Popover 首次悬停同步建窗)。定位新 FFI:`yue_mbt_view_root`(沿父链上溯顶层根,ViewStore::put 按 refcount 重持引用,与原句柄并存安全)+ `yue_mbt_view_origin_in_root_x/y`(沿途累加各层相对父 bounds),气泡与锚点同处根容器坐标系,水平居中 clamp 进根宽、下方放不下翻上方;挂锚点直接父容器会被后绘兄弟子树盖住(图标网格下一行),必须挂根且挂载序在锚点后(绘制在最上)。探针验证(A 区外按钮 + B scroll 内按钮,真实鼠标 8 步渐进 glide 驱动 + shim enter/leave 打点 + 抓屏):两锚点气泡均弹按钮正下方居中、长文本自适应宽、移开收起干净,打点序列 enter-B→leave-B→enter-A→leave-A 证明 libyue win hover 状态机对区外/scroll 内两区都无病。教训两条:①悬停点必须按实测控件矩形取中心——按估算值打到按钮右缘外 2px,enter 一次不触发,与「被遮挡不触发」症状同形,先核坐标再怀疑状态机;②打点日志追加模式跨轮累积会造出「同指针两次 enter」假象(同一信号多个槽各过一次蹦床也会让行数翻倍),每轮先清日志再看序列。真机联测续勘(用户真手操作 + shim 打点直读):气泡链路全健康——enter 触发后**精确 500ms** 弹出、origin_in_root 坐标正确(锚点 (20,20)→气泡 (0,54),水平 clamp 生效)、leave 即收;此前脚本轮「无气泡」是脚本侧问题(见下条方法论),非代码病。「tooltip 不会消失」的真因:滚轮滚动不移动鼠标,永远不产生 WM_MOUSEMOVE→leave 不触发→气泡浮在滚走的内容上。修复:attach_tooltip 挂**观察式滚轮**收气泡(见下下条)。
- 单行 Entry 吞滚轮·滚轮死区(真机联测定位,「输入框会屏蔽滚动」):Win32 把 WM_MOUSEWHEEL 投递给光标下的窗口——落在原生 Entry(RichEdit HWND)上时消息进控件自己的 wndproc;单行 EDIT/RichEdit 的 DefWindowProc 对滚轮既不自滚也不向父冒泡,页面就此锁死。libyue 的 `SubwinView::OnMouseWheelFromSelf` 本有转发骨架,但只在「事件不在控件矩形内」时转发,「在控件内」时默认控件会自己处理——对单行编辑控件正好落空。表单页大半面积是 Entry,真机表现为「整页滚不动,只能猛拨」,猛拨的连发滚轮再引发撕裂(见下条)。修复(fork 7f57e87f):SubwinView 以 `wants_wheel_`(构造时按 window_style 的 ES_MULTILINE 位测,仅多行编辑控件自滚)标记自滚意愿;非自滚控件的滚轮一律 `SendMessage` 给顶层窗口,走既有树派发(顶层 OnMouseWheel→content 树→ScrollImpl 先问光标下子树、无人消费则自滚),Entry 区域恢复滚动且不破坏 TextEdit 自身滚动手感。
- 连发滚轮撕裂与补画次序(真机「分裂」截图 + 猛拨残影截图,两阶段定案):**阶段一(mbt.14 病,撕裂)**:输入消息优先级高于 WM_PAINT——快拨时多个 WM_MOUSEWHEEL 在任何重绘之前排队,每个 OnScroll 先 ScrollWindowEx blit 再失效暴露边带,下一个 blit 的源区含着上一个未重画的边带,陈旧像素逐拍复合成大面错位。**阶段二(mbt.15 病,残影——首版修复插错了位置)**:为治阶段一把 `UpdateWindow` 插进 ScrollPixels,补画跑在 Layout **之前**——那一刻控件分配记录还是旧坐标,重绘遍历按旧坐标把自绘控件(input_t 外框/占位文字、按钮)画进表面烤住;真实输入连发,后续 blit 把烤好的旧影像一路搬运,滚动停下也留着(真机截图:「组内输入框」白框叠影列 + 顶部红按钮边缘线)。**终版(fork 87225dc3,mbt.16)**:补画收进 `ScrollImpl::PaintViewportNow`,固定在 blit → Layout(分配移位)之后调——边带重绘按新坐标画,同时保持同步补画防连发撕裂;SetOrigin/OnScroll 两条入口同改。注意 blit 读的是父表面,原生子控件(HWND)像素不在其上,烤影只可能来自「按过期分配重绘的自绘内容」——次序是唯一关键。方法论:①两张滚动后截图互比(连发 vs 慢滚)会双双带同一缺陷而「逐位一致」,必须对照「应有什么/不该有什么」的干净参照,真机截图一眼定案;②PostMessage WM_MOUSEWHEEL 直投 hwnd 可确定性复现连发时序(SendInput 注入不可靠——目标窗口非前台时整体被丢,脚本点击激活又可能落点窗外激活了别的窗口);③用户在场的真手联测效率远高于盲跑脚本:真手悬停/滚轮 + shim 文件打点 + 用户口述现象,一轮锁定根因。
- 消费式 on_wheel 会阻断滚动、观察式 FFI(上一条的配套):shim 既有 `yue_mbt_view_on_wheel` 是消费式——Windows 的 wheel_hook 回调后返回 true、GTK 蹦床返回 TRUE,事件不再冒泡(overlay thumb 窄条靠它独占滚轮再手动转发);挂在滚动区内的普通控件上会让页面滚不动。新增 `yue_mbt_view_on_wheel_observe`:Windows hook 返回 false、GTK 返回 FALSE,回调收 delta 后事件继续传播;tooltip 收气泡等旁路场景专用。注意 Windows 的 wheel_hook 是单槽,同一视图先后两种滚轮注册后写覆盖先写(buttons/icons 只挂观察式,无冲突)。
- Windows 切页/滚动横线与残影·fork 侧一刀根治(主窗口去 WS_CLIPCHILDREN):根因在顶层窗口风格 kWindowDefaultStyle 含 WS_CLIPCHILDREN——父表面在子窗口矩形处**从未被绘制**,UpdateChildBounds 递归下钻重摆 HWND 后旧位置露出陈旧像素,父窗口重绘时又被按当前子窗口集合裁剪,新旧交错区域永远不可达(横线/残影的本体)。本仓两次缓解均真机失败已回滚(48216a4 重绘风暴、2355d45 后的根 SchedulePaint)。根治:fork e373e60a 把 kWindowDefaultStyle 的 WS_CLIPCHILDREN 去位(kWindowDefaultChildStyle 不动,子控件自身无子不受影响)——BeginPaint 的 DC 不再挖子窗口矩形,失效区整块可画(画到子窗口底下被其表面盖住,DWM 恒按 Z 序合成父子表面,Win10 下安全),代价仅是被盖区域多画。验证:本地探针(双页容器显隐切页 + scroll + 3 原生 Entry,程序化切页/滚动按拍抓屏)对照——旧风格切页后横贯横线 + 页标题残影、滚动后横线,新风格四拍全净;像素级逐行梯度扫描四拍 top6 完全一致且全是结构性边界(标题栏/顶栏/底边),内容区零突变行;131 测全过、showcase 源码模式构建冒烟多页正常。接入:prepare.py backport_fork_main 追打补丁(源码模式生效);**预构建链(yue_prebuilt.lib)未含此修复**,fork 出新版本包(NATIVE_VERSION 更新)前的真机验证须走 LIBYUE_FORCE_SOURCE=1 源码模式构建。原「每原生子控件一个跟随裁剪窗」的 airspace 方案对横线不再必要(本刀已解),其仍可治「原生子控件滚动遮挡视口外」类问题(见滚动条条目),维持挂起。
- 滚动跟随系补丁整串回滚·用户拍板(fork 503366e4,mbt.17):真机多轮反馈定案——e373e60a(去 WS_CLIPCHILDREN)/f4528cb8(TranslateAllocation 平移)/cf308737(ScrollWindowEx blit)/2bf490f7+87225dc3(边带补画)整串在真机从未干净通过一轮(重影/悬浮/撕裂叠病),诊断开关(LIBYUE_SCROLL_FULL_INVALIDATE=1 整页重绘模式)下重影依旧,证明病灶在「按陈旧分配记录绘制」家族而非滚动搬运路径;用户拍板:**保留悬浮滚动条效果,撤销其后为表单页/元素悬浮所做的全部滚动系修复**。回滚后状态:①主窗口风格恢复 WS_CLIPCHILDREN(切页/滚动**横线会回归**,接受);②滚动模型回上游(全视口失效重绘)+ MoonBit 层第一代 0ms 重摆兜底恢复(declarative scroll()/overlay_scroll 的 on_scroll→set_timeout(0)→update_child_bounds)——表单页滚动**卡顿与快滚 1-2 帧蓝影回归**,接受;③保留:悬浮滚动条(attach_overlay_thumb)、单行 Entry 滚轮放行(fork 7f57e87f,独立修复无回归)、ba479418 切页递归下钻、tooltip 第三代窗口内自绘气泡(纯 MoonBit,含观察式滚轮收起)。prepare.py 的 backport 表中**已回滚补丁的条目必须保持删除**——mbt.17+ 源码包重新含有这些补丁的旧侧文本,条目残留会把回滚掉的补丁重新打上(见 backport_fork_main 注释)。后续若重启滚动优化,先读本条与撕裂/补画次序条:方向必须是「先保证分配记录新鲜(切页链),再动绘制模型」,且任何 fork 侧滚动补丁在真机干净通过一轮前不得叠加下一层。
- 滚动条形态:libyue 在 Windows 的滚动条是自绘经典样式(Scrollbar 类:轨道 + 箭头按钮 + 常驻占布局),`Scroll::SetOverlayScrollbar` 对 Windows 是空操作(头文件里 API 就被 `#if !defined(OS_WIN)` 排除)——GTK 的悬浮形态在 Windows 没有对应物。统一方案是 yue 层 `attach_overlay_thumb`:policy 置 Never 隐藏平台条,自绘 thumb 用 yoga absolute(right/top/bottom 静态样式)占满右缘全高,可见段在 on_draw 按 Ref 状态绘制——滚动更新只 schedule_paint 零 yoga 重排;浮现/渐隐用 clear_timeout 可取消的两级定时器(d9→73→隐藏),拖拽依赖按下后的隐式鼠标捕获(拖出仍收 move,见 splitter 条目的 WM_CAPTURECHANGED 适配),thumb 窄条上的滚轮经 on_wheel 手动转发给 Scroll。收口点:declarative `scroll()` 默认形态(overlay=true 且未显式 policy)即套 host 容器走它(style 参数挪给 host,Scroll 在内撑满——thumb 的 absolute 定位以 host 为基准,直接挂页面容器会被 padding/margin 带偏);overlay_scroll 组件与全站页面滚动同走此路径。另注(原生子控件滚动跟随·两代方案):Windows 上滚动平移只更新 content 容器自身的绝对坐标(`ScrollImpl::Layout` 只分配 content 的 Rect),其子孙里的原生子控件(Entry/TextEdit 等 HWND)不跟随——内容画走了、原生框停在旧屏幕位置盖住视口外区域(探针实测滚动 150px 后 Entry/TextEdit 屏幕坐标纹丝不动)。第一代修复(已退役):shim 暴露 `Container::UpdateChildBounds`,on_scroll 里以 0ms 定时器延到滚动布局完成后对 content 容器强制重摆子树——坐标跟随修好,但真机暴露两层代价:①**卡顿**:UpdateChildBounds 的 Layout→递归下钻与每层子容器的 SetBounds→再入 Layout 组合爆炸(表单结构探针实测每滚动拍 415 次 SubwinView::SizeAllocate、单次重摆 21ms),且每次 SizeAllocate 全套执行 SetWindowRgn/ShowWindow/RedrawWindow(RDW_ALLCHILDREN 全量失效,纯平移时全是无效功);②**蓝色残影碎片**:重摆延迟一整拍,快速滚动时原生控件在旧位置多显示 1-2 帧。第二代修复(fork f4528cb8,当前):滚动平移走专用零 Layout 传导——ViewImpl/ContainerImpl/SubwinView 三层 `TranslateAllocation`(纯平移记录+递归传播,SubwinView 复用带纯平移快路径的 SizeAllocate:尺寸与相对裁剪矩形都不变时只 SetWindowPos,窗口表面随移动搬运,零 region/show/redraw;进出视口边缘的控件仍走全套更新 region),ScrollImpl::Layout 对纯偏移直接 Translate、变尺寸才走原 SizeAllocate;MoonBit 层的 0ms 兜底已删(declarative scroll()/overlay_scroll)。探针复测:每拍 SizeAllocate 调用 415→41、0ms 兜底不再运行、快速滚动抓屏排布正常无蓝影。第三代(fork cf308737,真机复验仍卡后补):滚动重绘模型对齐 GTK——OnScroll/SetOrigin 用 `ScrollWindowEx`(视口裁剪内 blit 已有像素+SW_SCROLLCHILDREN 子窗口随移)只失效暴露边带,替代全视口失效重绘(内容重的页每帧全页重画是卡顿本体;去 CLIPCHILDREN 后父还要画到原生控件矩形底下,面积进一步放大),视口外控件跳过重复 ShowWindow(SW_HIDE)。探针:极限滚动(16ms 一拍 40px)全部链路合计 45% 单核、连拍无撕裂无蓝影;131 测全过。GTK3 对照事实:`gdk_window_scroll` 自 GTK3 起不做服务端拷贝而是失效交给 Cairo GPU 绘制,其流畅本质是控件全自绘(零 airspace)+GPU 绘制;Win32 侧 ScrollWindowEx 即等价最优模型。shim 的 update_child_bounds 接口保留(根级重算等非滚动场景兜底)。根治方向(已研究、暂缓):「每原生子控件一个跟随裁剪窗」——子窗口超出父客户区必然被系统裁剪(不依赖 SetWindowRgn,后者对子窗口的行为不可靠,实测调用参数正确而视觉不生效,时灵时不灵),裁剪窗只覆盖「控件矩形∩视口」故零新增遮挡;但实现要吃透 libyue 的 SubwinHolder/DPI/窗口偏移坐标体系(首次实现因窗口类未注册 1407、坐标基准错位等问题返工,远程盲调风险大),挂起待有真机调试条件时重启;**其中横线/残影症状已由主窗口去 WS_CLIPCHILDREN 根治(见上条),airspace 仅在仍需精确裁剪原生控件可视区时才有必要**。另注意全视口式 clipper 容器**不可行**:HWND 不绘制也遮挡,盖住视口内全部自绘内容(实测首页空白)。
